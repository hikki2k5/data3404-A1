...
from python_src.access.write.access_inserter import AccessInserter
...
class HeapFileInserter(AccessInserter):
    ...
    def __init__(self, first_page_id: PageId, buffer_manager: BufferManager, schema: TupleDesc):
    ...
    def insert(self, row: list) -> None:
        ...
        while self.current_page_id.is_valid():
            ...
            if heap_page.insert_record_auto(tuple_obj):
                self.buffer_manager.unpin(self.current_page_id, True)
                return
            
            next_page_id = heap_page.get_next_page_id()
            ...
            if not next_page_id.is_valid():
                ...
                new_page.initialise(heap_page.get_relation_name())
                new_page.set_previous_page_id(self.current_page_id)
